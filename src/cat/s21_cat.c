...
void initFlags(flags *items) {
  items->b = items->e = items->v = items->n = items->s = items->t =
      items->files = 0;
}

void initVariables(flags *items) {
  items->temp_f.n = items->temp_f.b = items->temp_f.s = items->temp_f.s1 =
      items->temp_f.text = 0;
}
...
void caseVET(flags *items, int string) {
  if (items->v && (string < 32 || string > 126) &&
      (string != 9 && string != 10)) {
    handleEndcoding(string);
  } else if (items->e && items->v && (string < 32 || string > 126) &&
             (string != 9)) {
    handleEndcoding(string);
  } else if (items->e && !items->v && string == 10) {
    handleEndcoding(string);
  } else if (items->t && items->v && (string < 32 || string > 126) &&
             (string != 10)) {
    handleEndcoding(string);
...
void caseNB(flags *items, FILE *streams[], int amount, int *currentStream) {
  ...
  if (items->n && items->files) {
    printf("%6d\t", items->line);
  } else if (items->n && !items->files &&
             (stream = handleGetStreams(streams, amount, currentStream))) {
    printf("%6d\t", items->line);
    if (stream == '\n') {
      items->temp_f.n = 1;
      items->line++;
